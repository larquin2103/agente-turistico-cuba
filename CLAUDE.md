...
**Deduplicación en seguimientos**: `tarjetas_enviadas` (dict en memoria `{usuario_id: {nombre_lower: timestamp}}`, TTL de 30 min vía `TARJETA_TTL_SEG`) registra cada tarjeta enviada en el flujo RAG bajo **dos claves**: el nombre detectado en la respuesta del LLM y el nombre resuelto en la DB (pueden diferir, p. ej. "Bodeguita del Medio" vs. "La Bodeguita del Medio"). En preguntas de seguimiento donde el LLM repite los mismos lugares, las tarjetas ya enviadas se omiten (`_tarjetas_recientes()` / `_registrar_tarjeta()`) y solo se muestran las nuevas. `/reset` limpia el registro del usuario, así que tras reiniciar la conversación las tarjetas vuelven a aparecer. Esto solo aplica al flujo RAG; los flujos de `/cercanos` siempre muestran sus tarjetas (el usuario las pidió explícitamente).
